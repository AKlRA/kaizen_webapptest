...
def register(request):
    if request.method == "POST":
        ...
        if role == "coordinator":
            if (
                User.objects.filter(profile__user_type="coordinator").count()
                >= 2
            ):
                return JsonResponse(
                    {
                        "success": False,
                        "error": "Maximum coordinator limit reached",
                    }
                )
        elif role == "finance":
            if User.objects.filter(profile__user_type="finance").count() >= 2:
                return JsonResponse(
                    {
                        "success": False,
                        "error": "Maximum finance users limit reached",
                    }
                )
        ...
        if role in ["employee", "hod"] and not department:
            return JsonResponse(
                {
                    "success": False,
                    "error": "Department is required for employees and HODs",
                }
            )
        ...
        try:
            with transaction.atomic():
                ...
                Profile.objects.create(
                    ...
                    user_type=role,
                    department=(
                        department if role in ["employee", "hod"] else None
                    ),
                    employee_id=user_id,
                )
    ...
    return render(
        request, "dashboard/register.html", {"departments": departments}
    )
...
def employee_dashboard(request):
    ...
    kaizen_sheets = KaizenSheet.objects.filter(
        employee=request.user, is_temporary=False
    ).order_by("-created_at")

    # Get available departments for horizontal deployment
    departments = (
        Profile.objects.filter(user_type="hod")
        .exclude(department=request.user.profile.department)
...
def view_sheet(request, sheet_id):
    ...
    sheet = get_object_or_404(
        KaizenSheet, id=sheet_id, horizontal_departments=request.user.profile
    )
...
def fetch_kaizen_sheet(request, kaizen_id):
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        for impact in impacts:
            ...
            impact_data[f"impacts_{impact}"] = getattr(
                kaizen, f"impacts_{impact}", False
            )
            ...
            try:
                if (
                    kaizen.cost_before_implementation
                    and kaizen.cost_after_implementation
                ):
        ...
        data = {
            ...
            "start_date": (
                kaizen.start_date.strftime("%Y-%m-%d")
                if kaizen.start_date
                else ""
            ),
            "end_date": (
                kaizen.end_date.strftime("%Y-%m-%d") if kaizen.end_date else ""
            ),
            ...
            # File URLs
            "before_improvement_image": (
                kaizen.before_improvement_image.url
                if kaizen.before_improvement_image
                else ""
            ),
            "after_improvement_image": (
                kaizen.after_improvement_image.url
                if kaizen.after_improvement_image
                else ""
            ),
            "standardization_file": (
                kaizen.standardization_file.url
                if kaizen.standardization_file
                else ""
            ),
            "cost_calculation": (
                kaizen.cost_calculation.url if kaizen.cost_calculation else ""
            ),
            "cost_benefits_description": impact_data.get(
                "cost_benefits_description", ""
            ),
            "cost_uom": impact_data.get("cost_uom", ""),
            "cost_before_implementation": impact_data.get(
                "cost_before_implementation", ""
            ),
            "cost_after_implementation": impact_data.get(
...
def get_department_stats(dept_name):
    ...
    hod = User.objects.filter(
        profile__department=dept_name, profile__user_type="hod"
    ).first()

    employee_count = Profile.objects.filter(
        department=dept_name, user_type="employee"
    ).count()
...
def coordinator_dashboard(request):
    ...
    today = timezone.localtime(
        latest_sheet.created_at if latest_sheet else timezone.now()
    )
    ...
    for year in range(start_year - 4, start_year + 1):
        ...
        monthly_average = (
            round(completed_sheets.count() / months_diff, 2)
            if months_diff > 0
            else 0
        )

        academic_years_data.append(
            {"year": f"AY {year}-{year + 1}", "average": monthly_average}
        )
    ...
    for month in range(1, 13):
        month_start = timezone.make_aware(
            timezone.datetime(current_year, month, 1)
        )
        month_end = timezone.make_aware(
            (
                timezone.datetime(current_year, month + 1, 1)
                if month < 12
    ...
    # Employee statistics
    employee_stats = []
    for user in User.objects.filter(profile__isnull=False).select_related(
        "profile"
    ):
        ...
        stats = {
            ...
            "approved": user_sheets.count()
            - user_sheets.filter(approval_status="pending").count(),
        }
    ...
    context = {
        ...
        "monthly_submissions": json.dumps(
            monthly_submissions, cls=DjangoJSONEncoder
        ),
        "monthly_completions": json.dumps(
            monthly_completions, cls=DjangoJSONEncoder
        ),
        ...
        "academic_year_counts": json.dumps(
            [d["average"] for d in academic_years_data], cls=DjangoJSONEncoder
        ),
        ...
        "completed_count": KaizenSheet.objects.filter(
            approval_status="completed"
        ).count(),
        "hod_pending_count": KaizenSheet.objects.filter(
            approval_status="pending"
        ).count(),
        "coordinator_pending_count": KaizenSheet.objects.filter(
            approval_status="hod_approved"
        ).count(),
        "finance_pending_count": KaizenSheet.objects.filter(
            approval_status="finance_pending"
        ).count(),
...
def get_department_data(request, year, month):
    ...
    try:
        ...
        for dept_name in department_names:
            ...
            # Get coordinator
            kaizen_coordinator_entry = KaizenCoordinator.objects.filter(
                department=dept_name
            ).first()
            coordinator_name = (
                kaizen_coordinator_entry.coordinator_name
                if kaizen_coordinator_entry
                else ""
            )
            ...
            monthly_target = employee_count
            achievement_rate = (
                (completed_sheets / monthly_target * 100)
                if monthly_target > 0
                else 0
            )
...
def cip_register_view(request):
    ...
    if not request.user.profile.is_coordinator:
        return HttpResponseForbidden(
            "Access denied. Coordinator privileges required."
        )

    try:
        kaizen_sheets = (
            KaizenSheet.objects.select_related("employee")
            .all()
            .order_by("-created_at")
        )
        ...
        for sheet in kaizen_sheets:
            ...
            hod = User.objects.filter(
                profile__user_type="hod", profile__department=department
            ).first()
            ...
            sheet_data = {
                ...
                # Processed URLs
                "standardization_file_url": (
                    sheet.standardization_file.url
                    if sheet.standardization_file
                    else None
                ),
                "cost_calculation_url": (
                    sheet.cost_calculation.url
                    if sheet.cost_calculation
                    else None
                ),
                "before_improvement_image_url": (
                    sheet.before_improvement_image.url
                    if sheet.before_improvement_image
                    else None
                ),
                "after_improvement_image_url": (
                    sheet.after_improvement_image.url
                    if sheet.after_improvement_image
                    else None
                ),
                "handwritten_sheet_url": (
                    sheet.handwritten_sheet.url
            ...
            for impact in impact_types:
                sheet_data["impact_data"][impact] = {
                    "benefits_description": getattr(
                        sheet, f"{impact}_benefits_description", ""
                    ),
                    "uom": getattr(sheet, f"{impact}_uom", ""),
                    "before_implementation": getattr(
                        sheet, f"{impact}_before_implementation", ""
                    ),
                    "after_implementation": getattr(
                        sheet, f"{impact}_after_implementation", ""
                    ),
...
def view_kaizen(request, kaizen_id):
    if not (request.user.profile.is_hod or request.user.profile.user_type == 'finance'):
        return HttpResponseForbidden()

    try:
        # Different query based on user type
        if request.user.profile.is_hod:
            department = request.user.profile.department
            try:
                sheet = KaizenSheet.objects.select_related("employee__profile").get(
                    id=kaizen_id, employee__profile__department=department
                )
                is_horizontal = False
        ...
        context = {
            ...
            "is_finance": request.user.profile.user_type == 'finance'
        }
...
def hod_dashboard(request):
    ...
    horizontal_sheets = (
        ...
        .exclude(employee__profile__department=department)
        .distinct()
        .order_by("-created_at")
    )
...
def upload_handwritten_sheet(request):
    if (
        request.method == "POST"
        and request.POST.get("form_type") == "handwritten"
    ):
        try:
            ...
            if not title or not handwritten_sheet:
                ...
                return JsonResponse(
                    {"success": False, "error": " and ".join(error_msg)}
                )
    ...
    return JsonResponse(
        {"success": False, "error": "Invalid request method or form type"}
    )
